...
int main(int argc, char *argv[])
{
	...
	gtk_widget_show(hbox4);GtkWidget *hbox5 = gtk_hbox_new(FALSE, 0);
...
void click_number(GtkWidget *widget, gpointer data)
{
	...
	if(entry_int == 0) gtk_entry_set_text(GTK_ENTRY(entry), button_num);
	else	;//gtk_entry_append_text(GTK_ENTRY(entry), button_num);
	has_press_number = 1;
	has_press_symbol = 0;
}
