...
int main (int argc, char *argv[])
{
	...
	label4 = gtk_label_new("라벨 4");
	

	hbox = gtk_box_new( GTK_ORIENTATION_HORIZONTAL, 5 );
	vbox = gtk_box_new (GTK_ORIENTATION_VERTICAL,10);
	
	gtk_box_pack_start(GTK_BOX(hbox), label1, TRUE,FALSE, 3);
	gtk_box_pack_start(GTK_BOX(hbox), label2, TRUE,FALSE, 3);

	gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE,FALSE, 5);
	gtk_box_pack_start(GTK_BOX(vbox), label3, FALSE,FALSE, 5);
	
	gtk_box_pack_start(GTK_BOX(vbox), label4, TRUE,FALSE, 5);
