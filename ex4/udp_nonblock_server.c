...
int main(int argc, char *argv[]) {
	...
	/* 서버 소켓에 대한 이벤트를 받을 프로세스 연결 */
	if (fcntl(sock, F_SETOWN, getpid()) < 0) {
		perror("Unable to set process owner to us");
		exit(1);
	}
	/* 서버 소켓을 넌블로킹&비동기 모드로 설정 */
...
void SocketHandler(int signalType) {
	...
	do {
		...
		if ((recvMsgSize = recvfrom(sock, buf,MAXBUFSIZE, 0, (struct sockaddr
		*)&clnt_addr,&clntLen)) < 0) {
			...
			if (sendto(sock, buf, recvMsgSize, 0,(struct sockaddr *)
			&clnt_addr,sizeof(clnt_addr)) != recvMsgSize) {
