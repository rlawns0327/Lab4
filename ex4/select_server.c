...
int main(int argc, char *argv[]) {
	...
	int sockfd, new_fd;
	/* listen on sockd, new connection on new_fd */
	struct sockaddr_in cad;
	/* structure to hold client's address */
	int alen;
	/* length of address */
	fd_set readfds, activefds;
	/* the set of read descriptors */
		...
		for (i = 0; i <= maxfd; ++i) {
			if (FD_ISSET (i, &readfds)) {
				if (i == sockfd) {
					...
					//add the new socket desc to our active connections set
					printf("set\n");
					if (new_fd > maxfd)
						maxfd = new_fd;
				} 
				else {
					/* Data arriving on an alreadyconnectedsocket. */
